...
- Seguridad de replay: el verifier rechaza `CACHE_STORE=null`, driver `null` y stores no definidos en producción antes de consultar JWKS; regresión parametrizada, preservando `array→file`.
...
| **BLOCKED / EXTERNAL** | 🚧 revisión final de PR sucesora | ⛔ no fusionar sin gates |
