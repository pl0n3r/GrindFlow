...
script_email="$(sed -nE 's/^E2E_USER_EMAIL=.*([a-z0-9._+-]+@grindflow[.]test).*$/\1/p' "$smoke_file")"
...
if [[ -z "$php_email" || "$php_email" == *# This contract is run by GrindFlow CI / fast without production credentials.
...
printf 'PASS production smoke contract: unknown\n'\n'* ||
      ...
      "$example_email" != "$php_email" ]]; then
  printf 'FAIL production smoke contract: dedicated OIDC identity drifted across runtime files.\n' >&2
  exit 1
fi
for runtime_file in "$workflow_file" "$config_file" "$smoke_file" "$env_example_file"; do
