...
run_case() {
  ...
  case "$label" in
    pending*)
      ...
      ! grep -Fq 'do-not-leak-csrf' "$log"; ! grep -Fq 'never-print' "$log"; ! grep -Fq "$NO_RETRY_MARKER" "$log"; ! grep -Eq '/(scheduler|distribution|traffic|finance)' "$requests";;
    current)
    ...
    current_vault_failure|current_vault_link_missing)
      grep -Fxq 'VAULT_READ_ONLY=failed' "$log"; grep -Fq 'ERROR: read-only Vault check failed on the current schema; no repeated login requests.' "$log"; ! grep -Fq "$NO_RETRY_MARKER" "$log";;
