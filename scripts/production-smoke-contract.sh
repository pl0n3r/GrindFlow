...
printf 'PASS production smoke contract: unknown\n' "$requests")" -eq "$expected_login_gets" ]] || { printf 'FAIL %s: expected %s read-only login GETs.\n' "$label" "$expected_login_gets" >&2; exit 1; }

  local expected_auth_class=""
  case "$auth_mode" in
    login_csrf_rotates) expected_auth_class="anonymous_session_inconsistent" ;;
    post_login) expected_auth_class="login_rejected_session_stable" ;;
    post_login_changed) expected_auth_class="login_rejected_session_changed" ;;
    post_login_unavailable) expected_auth_class="login_rejected_session_unavailable" ;;
    post_419) expected_auth_class="login_csrf_rejected" ;;
    post_429) expected_auth_class="login_rate_limited" ;;
    post_401|post_403|post_422) expected_auth_class="login_http_rejected" ;;
    post_301|post_307|post_308|post_external|post_network|post_absolute_scheme|post_absolute_port|post_absolute_host|post_absolute_userinfo)
      expected_auth_class="login_redirect_unrecognized"
      ;;
    dashboard_login|dashboard_absolute_login|dashboard_301|dashboard_303|dashboard_307|dashboard_308)
      expected_auth_class="authenticated_session_lost"
      ;;
    dashboard_external|dashboard_network|dashboard_absolute_external|dashboard_other|dashboard_secret)
      expected_auth_class="authenticated_redirect_unrecognized"
      ;;
    dashboard_401|dashboard_403|dashboard_419|dashboard_422|dashboard_429)
      expected_auth_class="authenticated_request_rejected"
      ;;
  esac
  if [[ -n "$expected_auth_class" ]]; then
    grep -Fxq "AUTH_FAILURE_CLASS=$expected_auth_class" "$log"
    [[ "$(grep -c '^AUTH_FAILURE_CLASS=' "$log")" -eq 1 ]]
  else
    assert_absent_regex '^AUTH_FAILURE_CLASS=' "$log"
  fi
