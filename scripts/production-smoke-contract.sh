...
python3 - \
  "$script_dir/../.github/workflows/production-smoke.yml" \
  "$script_dir/../app/Support/Deployment/ProductionEnvironmentWriter.php" \
  "$script_dir/../config/grindflow.php" \
  "$script_dir/production-smoke.sh" \
  "$script_dir/../.env.example" <<'PY'
...
smoke_script = Path(sys.argv[4]).read_text(encoding="utf-8")
environment_example = Path(sys.argv[5]).read_text(encoding="utf-8")

def one(pattern, contents, label):
    matches = re.findall(pattern, contents)
    assert len(matches) == 1, f"{label} must define exactly one smoke identity"
    return matches[0]

dedicated_email = one(
    r"public const DEDICATED_SMOKE_EMAIL = '([^']+)';",
    writer,
    "PHP writer",
)
runtime_emails = {
    one(r"(?m)^\s*E2E_USER_EMAIL:\s*([^\s#]+)\s*$", workflow, "workflow"),
    one(r'(?m)^E2E_USER_EMAIL="\$\{E2E_USER_EMAIL:-([^}]+)\}"for step_id in ("deployed_checkout", "synthetic_bootstrap"):
...
assert workflow.count('password-invalid|provision-phase-disabled|provision-password-missing') == 1, 'invalid writer password requires an explicit safe code'
for code in (
    ...
    assert code in workflow, f'provisioning failure missing from workflow allowlist: {code}'
print("PASS production smoke contract: dedicated OIDC identity stays synchronized")
...
printf 'PASS production smoke contract: unknown\n', smoke_script, "smoke script"),
    one(r"env\('SMOKE_USER_EMAIL', '([^']+)'\)", config, "Laravel config"),
    one(r'(?m)^SMOKE_USER_EMAIL="([^"]+)"for step_id in ("deployed_checkout", "synthetic_bootstrap"):
    assert re.search(rf"(?m)^      - id: {step_id}$", workflow), step_id
start = "      - name: Publish pre-login production failure\n"
end = "      - name: Validate smoke script\n"
assert workflow.count(start) == 1, "pre-login incident reporter must be unique"
...
assert re.fullmatch(r"[a-z0-9._+-]+@grindflow[.]test", dedicated_email), "dedicated smoke identity must use reserved synthetic domain"
assert runtime_emails == {dedicated_email}, "production smoke runtime identities drifted apart"
for runtime_source in (workflow, config, smoke_script, environment_example):
    assert "e2e-admin@grindflow.test" not in runtime_source, "legacy member-bound identity returned to smoke runtime configuration"
