...
run_case() {
  ...
  case "$label" in
    ...
    auth_post_login|auth_dashboard_login|auth_dashboard_other|auth_dashboard_secret|auth_post_303|auth_dashboard_303|auth_post_401|auth_post_403|auth_post_419|auth_post_422|auth_post_429|auth_dashboard_401|auth_dashboard_403|auth_dashboard_419|auth_dashboard_422|auth_dashboard_429)
...
# Exit explicitly: bare negated grep can fail silently before the PASS marker.
assert_absent_fixed() {
  if grep -Fq -- "$1" "$2"; then printf 'FAIL: forbidden diagnostic/request content.\n' >&2; exit 1; fi
}
assert_absent_regex() {
  if grep -Eq -- "$1" "$2"; then printf 'FAIL: forbidden diagnostic/request pattern.\n' >&2; exit 1; fi
}
