...
second_attempt_marker='Production smoke attempt 2/'
login_post_pattern='^POST http://mock/login
...
run_case() {
  ...
  case "$label" in
    pending*)
      ...
      ! grep -Fq 'do-not-leak-csrf' "$log"; ! grep -Fq 'never-print' "$log"; ! grep -Fq "$second_attempt_marker" "$log"; ! grep -Eq '/(scheduler|distribution|traffic|finance)' "$requests";;
    current)
    ...
    current_module_failure|current_csv_failure)
      ...
      ! grep -Fq "$second_attempt_marker" "$log"
      [[ "$(grep -c "$login_post_pattern" "$requests")" -eq 1 ]]
      ;;
    ...
    current_vault_failure|current_vault_link_missing)
      grep -Fxq 'VAULT_READ_ONLY=failed' "$log"; grep -Fq 'ERROR: read-only Vault check failed on the current schema; no repeated login requests.' "$log"; ! grep -Fq "$second_attempt_marker" "$log";;
