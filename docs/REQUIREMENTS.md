...
- Los eventos `complete` y `fail` heredan el destino del `prepare` vigente para conservar trazabilidad append-only. Tras migrar, un `prepare` legacy sin destino puede recibir una única nueva preparación con destino explícito, conservando el evento anterior; antes de migrar, el endpoint mantiene temporalmente el contrato v0.1.73 para despliegues parciales.
