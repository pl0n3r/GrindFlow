...
class GitHubActionsOidcVerifierTest extends TestCase
{
    ...
    #[DataProvider('unsafeProductionCacheStores')]
    public function test_production_oidc_rejects_null_or_undefined_cache_before_http(
        string $store,
        ?string $driver,
    ): void {
        $this->app->detectEnvironment(static fn (): string => 'production');
        ...
        if ($driver !== null) {
            config(["cache.stores.{$store}" => ['driver' => $driver]]);
        }

        $sha = str_repeat('a', 40);
        $this->expectException(RuntimeException::class);
        $this->expectExceptionMessage('A persistent cache store is required for production OIDC replay protection.');

        try {
            (new GitHubActionsOidcVerifier)->verify($this->token($sha), $sha);
        } finally {
            Http::assertNothingSent();
            self::assertSame($store, config('cache.default'));
        }
    }

    public static function unsafeProductionCacheStores(): array
    {
        return [
            'built-in null store' => ['null', null],
            'configured null driver' => ['custom-null', 'null'],
            'undefined store' => ['database', null],
            'empty store name' => ['', null],
        ];
    }
