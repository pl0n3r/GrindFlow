...
**v0.1.64, límite de intentos de contraseña:** máximo ocho solicitudes con CSRF válido por cuenta Symfony en una ventana móvil de quince minutos, compartidas entre sesiones del mismo usuario. La novena retorna `429` con `Retry-After` y `Cache-Control: no-store` antes de calcular un hash o tocar la base. El panel React limpia los campos también ante el bloqueo y muestra un error recuperable. Estado de rate limiter guardado en `cache.rate_limiter`: limpiar el cache reinicia cupos; un despliegue con varias instancias necesitaría cache común. Login conserva su límite independiente. MariaDB y navegador solo en CI descartable.
